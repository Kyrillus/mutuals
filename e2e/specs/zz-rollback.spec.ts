...
test('probe rollback', async ({ page }) => {
  ...
  page.on('request', (r) => { if (r.method() === 'PATCH') console.log('PATCH ->', r.url()) })
  page.on('requestfailed', (r) => { if (r.method() === 'PATCH') console.log('PATCH failed', r.failure()?.errorText) })
  page.on('response', (r) => { if (r.request().method() === 'PATCH') console.log('PATCH resp', r.status()) })
  ...
  for (const wait of [200, 500, 1000, 2000, 4000]) {
    ...
    console.log(wait, 'cell =', JSON.stringify(await page.getByRole('button', { name: 'Edit City' }).innerText()))
  }
