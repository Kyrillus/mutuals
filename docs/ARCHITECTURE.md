...
**Run conditions.** Apple M3, 16 GB, macOS 26.5.2. PostgreSQL 16.15 (aarch64, Debian) in Docker,
...
<!-- pnpm db:check --contacts=10000 --runs=3, 2026-09-03 -->

| Operator shape                            | Designed index      | Index actually used                       | Rows |    Median | Buffers |
| ----------------------------------------- | ------------------- | ----------------------------------------- | ---: | --------: | ------: |
| hydrate one page of 50 records            | `av_record_attr_uq` | `av_record_attr_uq`                       | 2666 |   1.14 ms |     926 |
| `short_text equals`                       | `av_attr_text_idx`  | `av_attr_text_idx`                        |   50 |   1.31 ms |  15,656 |
| `number between`                          | `av_attr_num_idx`   | `av_attr_num_idx`                         |   50 |   0.72 ms |  10,770 |
| `date after`                              | `av_attr_date_idx`  | `av_attr_date_idx`                        |   50 |   1.73 ms |  13,044 |
| `yes_no is yes`                           | `av_attr_bool_idx`  | `av_attr_bool_idx`                        |   50 |   6.31 ms |  75,137 |
| `single_select is one of`                 | `av_attr_opt_idx`   | `av_attr_opt_idx`                         |   50 |   2.82 ms |  50,488 |
| `tags contains any of`                    | `av_attr_key_idx`   | `av_record_attr_uq` &sup1;                |   50 |   1.00 ms |  10,626 |
| `short_text is empty`                     | `av_attr_rec_idx`   | `av_attr_rec_idx`                         |   50 |   0.09 ms |   1,043 |
| `short_text contains` (trigram)           | `av_trgm_idx`       | `av_trgm_idx`                             |   50 |   2.80 ms |  13,105 |
| `multi_select contains all of`            | `av_attr_opt_idx`   | `av_attr_rec_idx` &sup2;                  |   50 |   6.80 ms |  22,878 |
| `relation has any of`                     | `rl_reverse_idx`    | `rl_reverse_idx`                          |   50 |   1.20 ms |   5,449 |
| `ORDER BY` custom text, **asc**           | `av_attr_text_idx`  | `av_attr_key_idx` &sup3;                  |   50 |  10.73 ms |  53,042 |
| `ORDER BY` custom text, **desc**          | `av_attr_text_idx`  | `av_attr_key_idx` &sup3;                  |   50 |   8.87 ms |  53,042 |
| `ORDER BY warmth desc`                    | `cm_warm_idx`       | none — hash join + top-N heapsort &#8308; |   50 |   4.35 ms |   3,145 |
| **three chips + sort** (the real one)     | several             | `av_attr_text_idx` leads, two semi-joins  |   15 |   2.15 ms |  47,657 |
| row count over the same three chips       | several             | same                                      |    1 |   1.50 ms |  26,531 |
| quick search box (`q=`, two text columns) | `av_trgm_idx`       | `av_trgm_idx`                             |   50 |   3.18 ms |   7,562 |
| **delete one contact**                    | `record_pkey`       | `record_pkey` — see finding **F1**        |    0 | **4.0 s** |       8 |
...
tech" shape — is **2.2 ms at ten thousand contacts**, and the plan is the one the design predicted:
...
**Sorting by a custom attribute is the slowest read, at ~10 ms, and it is honest about why.** An
...
table: the same sort under the three-chip filter costs 2.2 ms. The escape hatch, if a sorted
...
1.0 ms. `yes_no is yes` cannot: it matches 4,950 of 10,000 records, so the planner materialises the
whole match set, does 4,950 `record_pkey` lookups to fetch `created_at`, and top-N sorts. 6 ms at
...
2. ² **`multi_select contains all of` uses `av_attr_rec_idx`.** `contains_all_of` compiles to a
   ...
   8.4 ms because it is evaluated per candidate record until 50 match. Correct, and worth
...
4. ⁴ **`ORDER BY warmth` uses no index at all.** The query drives from `record` and left-joins
   ...
   sorts, in 4.4 ms and 3,145 buffers. Cheap, and the right plan, but it is linear in the number of
   contacts and would be ~40 ms at 100k.
...
### F1 — Deleting one contact takes four seconds at 10k contacts. Four foreign keys have no index.

**Severity: high. It blocks §5.4 and §6.8.**
...
`pnpm db:check` creates exactly these four, under a `perf_tmp_` prefix, for the duration of its own
cleanup and drops them again — see `MISSING_FK_INDEXES` in `packages/db/src/seed/perf.ts`. That is a
deliberate signpost, not a fix: the schema the harness leaves behind is exactly the schema the
migrations describe.
