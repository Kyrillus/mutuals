...
export interface ReplaceInBatchInput {
  /** The *source column* whose cells are rewritten, by index. */
...
/**
 * §6.8 step 4's `Find & replace`, over one source column.
 *
 * It writes into `raw.edits`, the per-column overlay, and **not** into `mapped`. That is the whole
 * correctness argument: `mapped` is derived from `raw` and is regenerated by every re-map, so a
 * replacement written there survives until the next mapping change and then silently vanishes. It
 * is also what makes a replaced cell revertible — `revertImportRow` clears the overlay, and the
 * file's own value is still underneath.
 *
 * Done in SQL rather than by reading every row into the API and writing it back: on a 10k-row import
 * that is 10k round trips for what is one `UPDATE`. The caller re-derives the rows it changed,
 * because validation needs the attribute registry and that lives one layer up.
 */
export async function replaceInImportBatch(
  ...
  const key = String(input.columnIndex)

  /**
   * The cell as it stands: the edit overlay if there is one, otherwise the file's own value.
   *
   * Repeated in the `SET` and the `WHERE` rather than factored into a SQL function, because a
   * function would be a migration and a schema object for two uses in one statement. The casts are
   * load-bearing: `->>` is overloaded on text and integer, and an untyped parameter leaves Postgres
   * unable to choose.
   */
  const cell = sql<string>`coalesce(
    raw -> 'edits' ->> ${key}::text,
    raw -> 'cells' ->> ${input.columnIndex}::int,
    ''
  )`

  /**
   * `edits` is created before writing into it.
   *
   * `jsonb_set` only creates the *last* element of its path, so on a row that has never been edited
   * the path `{edits, 3}` cannot be traversed and it returns the row unchanged — with no error and
   * a perfectly normal `UPDATE ... RETURNING` reporting the row as touched. That failure is
   * completely silent; it took an assertion on the resulting value to see it at all.
   */
  const rows = await sql<{ row_number: number }>`
    update import_row
       set raw = jsonb_set(
             raw || jsonb_build_object('edits', coalesce(raw -> 'edits', '{}'::jsonb)),
             array['edits', ${key}::text],
             to_jsonb(regexp_replace(${cell}, ${needle}, ${input.replace}, 'g')),
             true
           )
     where batch_id = ${batchId}
       and ${cell} ~ ${needle}
