...
pnpm db:check     # the 10k x 60 read measurement, ~2 min. Not part of verify.
pnpm screenshots  # regenerate docs/screenshots/ against a seeded database
```

Two measurements are opt-in because they take minutes and would make CI useless:

```bash
MUTUALS_IMPORT_PERF=1 pnpm test:integration   # R5: a 10,000-row import, ~5 min (ADR-117)
MUTUALS_LLM_LIVE=1 pnpm test:unit             # ADR-072's live smoke test; spends money
```

Both skip loudly when their variable is absent, so a green run says which measurements it did not
make. `POOLER_DATABASE_URL` does the same for §13's R7.
...
7. ~~**Polish and `v0.1.0`**~~ — empty states everywhere, the keyboard and accessibility pass,
   screenshots regenerated by `pnpm screenshots`, the README rewritten for release, the read path
   re-measured and the **write** path measured for the first time (§13's R5), and the tag.
   **Done**, in PR #1 (ADR-089); ADR-115 to ADR-117 record what it settled.

**Phase 1 is complete.** What comes next is §9's list — semantic search, nudges, chat channels,
Gmail sync — and none of it starts without a conversation first. Each stage ended with green CI,
updated docs, a PR and a stop for approval; that rhythm is worth keeping.
